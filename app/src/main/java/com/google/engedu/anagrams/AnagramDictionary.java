...
public class AnagramDictionary {
    ...
    public AnagramDictionary(Reader reader) throws IOException {
        ...
        while ((line = in.readLine()) != null) {
            ...
            boolean keyExists = lettersToWords.containsKey(sortedInputWord);
            if (keyExists) {
                lettersToWords.get(sortedInputWord).add(word);
            } else {
                ArrayList<String> words = new ArrayList<>();
                words.add(word);
                lettersToWords.put(sortedInputWord, words);
            }
  ...
  */
    public ArrayList<String> getAnagramsWithOneMoreLetter(String word) {
        ArrayList<String> result = new ArrayList<String>();
        String wordwithoneletter = "";
        for (char i = 'a'; i <= 'z'; i++) {
            wordwithoneletter = sortLetters(word.concat("" + i));
            if (lettersToWords.containsKey(wordwithoneletter)) {
                result.addAll(lettersToWords.get(wordwithoneletter));
            }
        }
        return result;
    }
